...
if uploaded_file is not None:
    ...
    def calculate_summary(df, remark_types):
        ...
        for (date, cycle), group in df_filtered.groupby(['DATE', 'CYCLE']):
            ...
            call_drop_count = group[(group['STATUS'].str.contains('NEGATIVE CALLOUTS - DROP CALL', na=False)) & (~group['REMARK BY'].str.upper().isin(['SYSTEM']))]['ACCOUNT NO.'].count()
        ...
        return summary_table.sort_values(by=['DATE', 'CYCLE'])

    for cycle in df['CYCLE'].unique():
        st.write(f"## Summary for Cycle {cycle}")
        df_cycle = df[df['CYCLE'] == cycle]
        
        st.write("### Predictive Summary Table")
        st.write(calculate_summary(df_cycle, ['Predictive', 'Follow Up']))
        
        st.write("### Manual Summary Table")
        st.write(calculate_summary(df_cycle, ['Outgoing']))
